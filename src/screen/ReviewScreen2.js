...
const ReviewScreen2 = (props) => {
    ...
    const reviewInfoApi = () => {
        Api.send('review_info', {"idx":params.idx}, (args)=>{

            let resultItem = args.resultItem;
            let arrItems = args.arrItems;
    
            if(resultItem.result === 'Y' && arrItems) {
               console.log('작성한 리뷰정보 가져오기:::: ', arrItems);
               //setExpertView(arrItems);
               setScore1(arrItems.score1 - 1);
               setScore2(arrItems.score2 - 1);
               setScore3(arrItems.score3 - 1);
               setScore4(arrItems.score4 - 1);
               setScore5(arrItems.score5 - 1);
               setMemo1(arrItems.review_private);
               setMemo2(arrItems.review_open);
            }else{
               console.log('작성한 리뷰정보 가져오기 실패!!!!', resultItem);
    ...
    const reviewWriteApi = () => {
        Api.send('review_update', {'idx':params.idx, "score1":score1, "score2":score2, "score3":score3, "score4":score4, "score5":score5, "review_private":memo1, "review_open":memo2}, (args)=>{

            let resultItem = args.resultItem;
            let arrItems = args.arrItems;
    
            if(resultItem.result === 'Y' && arrItems) {
               console.log('이사전문가 리뷰수정완료: ', resultItem);
               //setExpertView(arrItems);
    ...
    return (
        <Box flex={1} backgroundColor='#fff'>
            <SubHeader navigation={navigation} headerTitle='후기 수정' />
            <ScrollView>
            {
                reviewpage == "1" && 
                <Box px='25px' py='20px'>

...
const ExpertArea = (props) => {
    ...
    return (
        <Box flex={1} backgroundColor='#fff'>
            <SubHeader headerTitle="이사가능지역 선택" navigation={navigation} />
            {
                loading ?
                                            ...
                                            areaData2.map((item, index) => {
                                                return(
                                                    <TouchableOpacity 
                                                        ...
                                                        style={[styles.areaButton2, (index + 1) % 3 == 0 ? {marginRight:0} : {marginRight:(width - 50) * 0.034}, selectArea2.includes(item.alias + " " + item.wr_2) && {backgroundColor:'#0195ff', borderColor:'#0195ff'}]}
                                                    >
                                                        <DefText text={item.wr_2} style={[styles.areaButtonText, selectArea2.includes(item.alias + " " + item.wr_2) && {color:'#fff'}]} />
                                                    </TouchableOpacity>
                                                )
                                            })
                                        }
            ...
            <BottomButton 
                ...
                rightDisable={ selectArea2.length > 0 ? false : true } 
                rightBtnStyle={ selectArea2.length > 0 ? colorSelect.sky : colorSelect.gray }  

...
import { TouchableOpacity, StyleSheet, Dimensions, Image, ScrollView, Alert } from 'react-native';
...
const ExpertMessageView = (props) => {
    ...
    const onSend = useCallback((messages = []) => {
        ...
        ApiExpert.send('chat_chatInsert', {"idx":params.idx, "aidx":params.aidx, "atype":params.atype,"id":userInfo.ex_id, "content":messages[0].text, 'mname':userInfo.ex_name}, (args)=>{

            let resultItem = args.resultItem;
            let arrItems = args.arrItems;
    
            if(resultItem.result === 'Y' && arrItems) {
               console.log('채팅 글쓰기 보기: ', arrItems, resultItem);
    ...
    useEffect(()=> {
        //console.log('???????');
        Api.send('member_exchatCntRefresh', {'ex_id':userInfo.ex_id, 'cidx':params.idx}, (args)=>{
            console.log(":::::::::::::::::::::",args);
        });
    },[messages])

    const chatListApi = () => {
        ApiExpert.send('chat_chatList', {"idx":params.idx}, (args)=>{
    ...
    const _imgSendButton = (photo) => {
        ApiExpert.send('chat_chatInsert', {"idx":params.idx, "aidx":params.aidx, "atype":params.atype,"id":userInfo.ex_id, 'mname':userInfo.ex_name, 'upfile':photo}, (args)=>{

            let resultItem = args.resultItem;
            let arrItems = args.arrItems;
    
            if(resultItem.result === 'Y' && arrItems) {
               //console.log('채팅 글쓰기 보기: ', arrItems);
              // setChatList(arrItems);
              //setMessage(arrItems)
              setMessage(arrItems)
            }else{
    ...
    const fileSendApi = (file) => {
        ...
        ApiExpert.send('chat_chatInsert', {"idx":params.idx, "aidx":params.aidx, "atype":params.atype,"id":userInfo.ex_id, 'mname':userInfo.ex_name, 'upfile':file}, (args)=>{
    ...
    useEffect(()=>{
        messaging().onMessage((remoteMessage) => { 
            ...
            console.log('채팅apiapi....expert:::', remoteMessage);
            chatListApi();
        });
    },[])

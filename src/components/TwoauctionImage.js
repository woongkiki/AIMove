...
const TwoauctionImage = (props) => {
    ...
    const reservationButtion = (idx, aucidx, exid, exname) => {
        console.log(idx, aucidx, exid, exname);
    ...
    const twoAuction = async () => {
        ...
        //진행중인 사진으로 이사 견적요청
        await Api.send('tworoom_auction', {'mid':userInfo.id}, (args)=>{
            ...
            if(resultItem.result === 'Y' && arrItems) {
                ...
                console.log('가정집이사 견적요청 리스트 불러오기 성공!', arrItems, resultItem);
                setVisitList(arrItems);
                //setVisitList([]);
               
            }else{
                console.log("리스트 실패", resultItem);
                setVisitList([]);
            }
        });
    ...
    const twoExpertListApi = (auc_idx) => {

        console.log("auc_idx", auc_idx);

         //경매에 참여한 전문가리스트
        if(auc_idx != ""){
            Api.send('tworoom_auctionExpert', {'auc_idx':auc_idx}, (args)=>{
                ...
                if(resultItem.result === 'Y' && arrItems) {
                    ...
                    console.log('견적 참여한 전문가 리스트 불러오기 성공!', arrItems, resultItem);
                    setVisitExpert(arrItems);
                }else{
                    console.log("견적 참여한 전문가 불러오기 실패", resultItem);
                    setVisitExpert([]);
                }
            });
        }
    ...
    const twoAuctionReservation = () => {
        Api.send('tworoom_reservation', {'auc_idx':visitList.idx, "ex_id":reservationexid, "ex_idx":reservationIdx, "ex_name":reservationexname, "mid":userInfo.id}, (args)=>{
            ...
            if(resultItem.result === 'Y' && arrItems) {
                ...
                console.log('가정집이사 예약하기 성공!', resultItem);
               ...
               console.log('가정집이사 예약하기 실패!', resultItem);
              
            }
        });
    }

    const twpAuctionCancleApi = () => {
        Api.send('tworoom_auctionCancle', {'idx':visitList.idx, 'mid':userInfo.id}, (args)=>{
    ...
    useEffect(()=> {
        if(visitList != ""){
            twoExpertListApi(visitList.idx);
        }
    }, [visitList])
                ...
                <Box>
                {
                    ...
                    <>
                        <Box p='25px' py='15px'>
                            ...
                            <TouchableOpacity onPress={()=>navigation.navigate("TwoRoomConfirmMy", visitList)} style={{borderRadius:5, alignItems:'center', justifyContent:'center', height:45, marginTop:10, backgroundColor:'#0195ff'}}>
                    ...
                    <Box alignItems='center' flex={1} py='40px'>
                        <DefText text="요청한 가정집이사 견적내역이 없습니다." />
                    </Box>
                }
                </Box>
            }
                ...
                <Modal.Content width={width-50} backgroundColor='#fff'>
                    <Modal.Body px='20px' py='20px'>
                        ...
                        <HStack justifyContent={'space-between'} mt='20px'>
                            <TouchableOpacity onPress={twoAuctionReservation} style={[styles.modalButton]}>
            ...
            <Modal isOpen={reservationCancleModal} onClose={()=>setReservationCancleModal(false)}>
                <Modal.Content width={width-50} backgroundColor='#fff'>
                    <Modal.Body px='20px' py='20px'>
                        ...
                        <HStack justifyContent={'space-between'} mt='20px'>
                            <TouchableOpacity onPress={twpAuctionCancleApi} style={[styles.modalButton]}>
                                <DefText text="예" style={[styles.modalButtonText]} />
                            </TouchableOpacity>

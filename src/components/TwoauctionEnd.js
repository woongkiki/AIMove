...
const TwoauctionEnd = (props) => {
    ...
    const twoAuctionEndListApi = async () => {
        await setLoading(true);
        await Api.send('house_endList', {'mid':userInfo.id}, (args)=>{

            let resultItem = args.resultItem;
            let arrItems = args.arrItems;
    
            if(resultItem.result === 'Y' && arrItems) {
               console.log('가정집이사 방문견적 이사 완료 리스트 보기: ', arrItems, resultItem);
               setEndList(arrItems);
            }else{
               console.log('가정집이사 방문견적 이사 완료 리스트 실패!', resultItem);
